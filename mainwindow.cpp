...
//帮助菜单
    ...
    refreshButton->setIcon(QIcon(":/icons/refresh.png"));  // 如果有刷新图标
    ...
    layout->addLayout(buttonLayout, 0, 0);  // 添加到顶部
    ...
    //添加属性示例
    // attributes << "classMajor VARCHAR(20)";
    // insertAttribute("test1", "testtable1", attributes);

    //修改属性示例
    // attributes;
    // attributes << "ClassMajor VARCHAR(25)" << "classMajor VARCHAR(20)";
    // alterAttribute("test1", "testtable1", attributes);

    //删除属性示例
    // attributesToDelete;
    // attributesToDelete << "classMajor VARCHAR(20)";
    // deleteAttribute("test1", "testtable1", attributesToDelete);
...
void mainwindow::createEmptyTable(QString dbName, QString tableName)
{
    ...
    if (tableFile.exists()) {
        ...
        if (tableFile.open(QIODevice::WriteOnly | QIODevice::Text)) {
            ...
            tableInfo["attributes"] = QJsonArray(); // 空属性列表
            tableInfo["data"] = QJsonArray(); // 空数据
...
void mainwindow::alterAttribute(QString dbName, QString tableName, QString attribute)
{
    ...
    // 解析传入的属性定义
    // 格式应为 "属性名 类型定义"，如 "academy varchar(25)"
    ...
    QString attrName = parts[0]; // 提取属性名
    QString newTypeDef = parts.mid(1).join(' '); // 合并剩余部分作为类型定义
...
void mainwindow::deleteAttribute(QString dbName, QString tableName, QStringList attributes)
{
    QString tableFilePath = "D:\\数据库实训\\DBMS\\Users\\" + userName + ".u\\ruanko.db\\" + dbName + ".db\\" + tableName + ".table";
    ...
    for (const QString &attrToDelete : attributes) {
        bool attrFound = false;
        // 从属性列表中删除
        for (int i = 0; i < attributesArray.size(); ++i) {
            if (attributesArray[i].toString() == attrToDelete) {
        ...
        if (!attrFound) {
            qDebug() << "属性" << attrToDelete << "不存在，跳过删除";
            continue;
        }
        ...
        for (int i = 0; i < dataArray.size(); ++i) {
            QJsonObject row = dataArray[i].toObject();
            if (row.contains(attrToDelete)) {
                row.remove(attrToDelete);
                dataArray[i] = row;
            }
        }
    }
...
void mainwindow::processDDL()
{
    ...
    // 检查是否已选择数据库
    if (dataBase.isEmpty() && !lowerCommand.startsWith("create database")) {
        dialogEdit->append("ERROR: No database selected. Please use 'USE database' first.");
        QTextCursor cursor = dialogEdit->textCursor();
        cursor.movePosition(QTextCursor::End);
        dialogEdit->setTextCursor(cursor);
        dialogEdit->append("mysql<");
    ...
    //创建表
        ...
        // 解析 CREATE TABLE 语句
        // 新格式: CREATE TABLE 表名 (列名1 类型1, 列名2 类型2, ...)
    ...
    //修改表
        ...
        if (operation == "add") {
            ...
            deleteAttribute(dataBase, tableName, attributes);
            dialogEdit->append("Query OK, 0 rows affected");
        }
        else {
    ...
    //插入数据
        ...
        // 解析 INSERT INTO 语句
        // 新格式: INSERT INTO table VALUES (value1, value2, ...)
